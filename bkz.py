...
def check_linear_predicate(vector, lattice):
    ...
    m= len(t) #dimension of vector -2 = number of signatures
    ...
    if last == -tau:
        alpha = inv_t0 * (a[0] + (vector[m] + omega) % q) % q
        for i in range(1,m):
...
def scan_vectors_with_predicate(lattice,
    ...
    # best_lifts() returns a list of tuples:
    # [ (sqnorm, (ell_idx, r_idx), coeffs), ... ]
    lifts = g6k.best_lifts()

    print(f"[+] Scanning up to {max_vectors} vectors for the predicate...")

    count = 0
    for sqnorm, proj_range, coeffs in lifts:
        if count >= max_vectors:
            break

        # coeffs is a vecotr of Z^d over the generators B[0..D-1]
        # B.multiply_left(coeffs) returns the vector over the canonical coordinates.
        print("qui")
        v = B.multiply_left(coeffs)
...
def predicate_attack(lattice,
    ...
    d = len(lattice.signatures) + 2


    # Initialize G6K and sieve parameters
    g6k = Siever(lattice.B)

    #tracer = dummy_tracer(g6k)
    g6k.shrink_db(0)
    g6k.lll(0, d)
    g6k.update_gso(0, d)
    ...
    g6k.initialize_local(kappa, ell, d) #ho tolto il tracer
    ...
    while ell >= 0 and ell_steps < max_ell_iters:
        ...
        for loop in range(max_loops_per_ell):
            ...
            found, v_solution, alpha = scan_vectors_with_predicate(lattice, g6k, lattice.B)
            if found:
                if verbose:
